...
add_observations_linear <- function(whaledf, start_time, end_time,
  ...
  # track previous accepted Time_Start
  last_start <- -Inf
  
  for (k in seq_len(n)) {
    i <- whaledf$Time_Start[k]
    
    # enforce spacing between accepted Time_Start values
    if ((i - last_start) < jump_size) {
      next
    }
    ...
    whale <- as.integer((whale_csum[j] - whale_csum[k]) > 0)
  ...
  results <- results %>%
    data.frame() %>%
    tidyr::drop_na() %>%
    dplyr::mutate(Humpback = as.factor(Humpback))
...
add_observations_linear_tolerance <- function(whaledf, start_time, end_time,
                                    num_time_steps = 40, numcep = 12,
                                    step_size, jump_size = 0.1,
                                    tol = 1e-9) {
  
  whaledf <- whaledf %>%
    dplyr::filter((Time_Start > start_time - tol) & (Time_Start < end_time - tol)) %>%
    dplyr::arrange(Time_Start)
  ...
  if ((start_time < min(whaledf$Time_Start) - tol) || 
      (max(whaledf$Time_End) < end_time - tol)) {
    stop("Specified time range is not within data time range")
  }
  ...
  for (k in seq_len(n)) {
    ...
    # enforce spacing between accepted Time_Start values
    if ((i - last_start) < jump_size - tol) {
      next
    }
    
    window_end <- i + window_len
    
    if (window_end > end_time + tol) {
      break
    }
    
    # advance right pointer only forward
    while (j <= n && whaledf$Time_Start[j] < window_end - tol) {
      j <- j + 1L
    }
